...
- (NSColor *)toUIColor
{
    ...
    if ([self length] == 9)
    {
        return [NSColor colorWithRed:((c & 0xff000000) >> 24)/255.0 green:((c & 0xff0000) >> 16)/255.0 blue:((c & 0xff00) >> 8)/255.0 alpha:(c & 0xff)/100.0];
    }
    else if ([self length] == 8)
    {
        return [NSColor colorWithRed:((c & 0xff00000) >> 20)/255.0 green:((c & 0xff000) >> 12)/255.0 blue:((c & 0xff0) >> 4)/255.0 alpha:(c & 0xf)/100.0];
    }
    else if ([self length] == 7)
    {
        return [NSColor colorWithRed:((c & 0xff0000) >> 16)/255.0 green:((c & 0xff00) >> 8)/255.0 blue:(c & 0xff)/255.0 alpha:1.0];
    }
...
- (id)init
{
	if (self = [super init]) {
        ...
        _toUIColorRegex = [NSRegularExpression regularExpressionWithPattern:@"\\[@\"#*.{0,}\\stoUIColor]" options:0 error:NULL];
...
- (NSColor *)colorInText:(NSString *)text selectedRange:(NSRange)selectedRange type:(OMColorType *)type matchedRange:(NSRangePointer)matchedRange
{
    ...
    if (!foundColor) {
        NSTextCheckingResult *result = [_toUIColorRegex firstMatchInString:text options:NSMatchingReportCompletion range:NSMakeRange(0, text.length)];
        NSString *v = [text substringWithRange:result.range];
        foundColorRange = result.range;
        NSRange r = [v rangeOfString:@"\"#"];
        NSRange r2 = [v rangeOfString:@"\"" options:NSBackwardsSearch];
        if (r2.location > r.location + r.length)
        {
            NSString *c = [v substringWithRange:NSMakeRange(r.location + r.length - 1, r2.location - r.location - r.length +1)];
            foundColor = [c toUIColor];
            foundColorType = OMColorTypeToUIColor;
        }
